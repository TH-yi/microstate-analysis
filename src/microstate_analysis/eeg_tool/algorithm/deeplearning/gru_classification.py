...
if __name__ == '__main__':
    ...
    path_save_dir = r'/media/concordia/DA081B82081B5D37/Hongjiang/EEG/gru/seq_100_0.75'
    path_save_dir_cl = r'/media/concordia/DA081B82081B5D37/Hongjiang/EEG/gru/seq_100_0.75_cl_fnn'
    ...
    for subject in subjects:
        ...
        # indices = np.arange(len(dataset_target_train))
        # idx = np.random.shuffle(indices)
        # dataset_target_train = dataset_target_train[idx.astype(int)]
        # dataset_feature_train = dataset_feature_train[idx.astype(int)]
        seed = random.random()
        random.seed(seed)
        ...
        classifier = ClassifierCNN(6,len(conditions)).to(device)
        classifier.apply(init_weights)
        parameters_count[subject] = count_parameters(classifier)
        write_append_info(sub_path_cl, parameters_count_path_cl, parameters_count)
        ...
        for epoch in range(N_EPOCHS):
            if restart_time:
                ...
                restart_time = False
            train_loss, train_accuracy, output_label = train_classifier(classifier, dataset_feature_train, dataset_target_train, CLIP)
            end_time = time.time()
            epoch_mins, epoch_secs = epoch_time(start_time, end_time)
            if train_loss < best_train_loss:
                best_train_loss = train_loss
                label_save['output_train'] = output_label
                label_save['label_train'] = dataset_target_train
                torch.save(classifier.state_dict(), models_path_cl)
            if epoch % 10 == 0:
                ...
                write_append_info(sub_path_cl, epochs_info_path_cl, epoch_info)
                restart_time = True
                ##valid_loss1, valid_accuracy1, output_label_val1 = evaluate_classifier(classifier, dataset_feature_test,
                #                                                                   dataset_target_test)
                #print(f'\t Val. Loss_test: {valid_loss1:.3f} |  Val. Accuracy_test: {valid_accuracy1:.3f}')

        classifier.load_state_dict(torch.load(models_path_cl))
        valid_loss, valid_accuracy, output_label_val = evaluate_classifier(classifier, dataset_feature_test, dataset_target_test)
        epoch_info = {'valid_loss': valid_loss, 'valid_accuracy': valid_accuracy}
        write_append_info(sub_path_cl, valid_path_cl, epoch_info)
        print(f'\t Val. Loss: {valid_loss:.3f} |  Val. Accuracy: {valid_accuracy:.3f}')
        label_save['output_test'] = output_label_val
        label_save['label_test'] = dataset_target_test
