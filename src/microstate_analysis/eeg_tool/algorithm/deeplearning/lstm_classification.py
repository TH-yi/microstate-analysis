...
class ClassifierCNN(nn.Module):
    def __init__(self, in_c, n_classes):
        super().__init__()
        self.conv1 = nn.Conv1d(in_c, 32, kernel_size= 3, stride=1, padding=1)
        self.bn1 = nn.BatchNorm1d(32)
        # self.conv2 = nn.Conv1d(32, 64, kernel_size= 3, stride=1, padding=1)
        # self.bn2 = nn.BatchNorm1d(64)
        self.fc1 = nn.Linear(64*32,n_classes).cuda()
    ...
    with torch.no_grad():
        for i,_ in enumerate(data):
            ...
            for model_index, condition in enumerate(conditions):
                ...
                hidden_all[:,:,model_index] = hidden_layer_2
...
if __name__ == '__main__':
    ...
    path_save_dir_cl = r'/media/concordia/DA081B82081B5D37/Hongjiang/EEG/seq_200_0.75_cl'
    ...
    for subject in subjects:
        ...
        for epoch in range(N_EPOCHS):
            if restart_time:
                ...
                restart_time = False
            train_loss, train_accuracy = train_classifier(classifier, dataset_feature_train, dataset_target_train, CLIP)
            end_time = time.time()
            epoch_mins, epoch_secs = epoch_time(start_time, end_time)
            if train_loss < best_train_loss:
                best_train_loss = train_loss
                torch.save(model.state_dict(), models_path_cl)
            if epoch % 10 == 0:
        ...
        model.load_state_dict(torch.load(models_path_cl))
        valid_loss, valid_accuracy = evaluate_classifier(classifier, dataset_feature_test, dataset_target_test)
