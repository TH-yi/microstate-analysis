...
def calculate_hidden(model,data):
    model.eval()
    hidden = []
    with torch.no_grad():
        for i,_ in enumerate(data):
            src = data[i]
            output,_,_ = model.encoder(src)
            hidden_layer_2 = output[:,1::,:]
            hidden.append(hidden_layer_2)
    return hidden
...
# print html
def print_color(t):
    html_object = html_print(''.join([cstr(ti,color=ci) for ti,ci in t]))
    display(html_object)
    return html_object
...
# get approprite color for value
def get_clr(value):
...
def visualize(hidden_values, input, unit_no, pic_path, htmlname, figname):
    print("\n unit number:", unit_no,"\n")
    text_colors = []
    for i,_ in enumerate(input):
        subpath = os.path.join(pic_path,'data'+str(i))
        if not os.path.exists(subpath):
            os.makedirs(subpath)
        htmlpath = os.path.join(subpath, htmlname)
        figpath = os.path.join(subpath, figname)
        src = input[i]
        src = src[:, 1::, :]
        hidden = hidden_values[i]
        feature_size = src.shape[-1]
        hidden_size = hidden.shape[-1]
        scrmax = src.reshape(-1, feature_size).argmax(1)
        hidden = hidden.reshape(-1,hidden_size)
        maxlist = scrmax.tolist()
        slist = [chr(ord('@')+(value+1)) for value in maxlist]
        hlist = hidden[:,unit_no].tolist()
        hlist_n = normalization(hlist)
        for j in range(len(slist)):
            text = (slist[j], get_clr(hlist_n[j]))
            text_colors.append(text)
        html_object = print_color(text_colors)
        with open(htmlpath,'w') as file:
            file.write(html_object.data)
        text_colors = []
        imgkit.from_file(htmlpath,figpath)
...
if __name__ == '__main__':
    ...
    path_savepic_dir = r'/media/concordia/DA081B82081B5D37/Hongjiang/EEG/seq_200_0.75_show'
    ...
    for subject in subjects:
        ...
        hidden = calculate_hidden(model, data_set.train_data)

        subject = subjects[0]
        subpath_s = os.path.join(path_savepic_dir, subject)
        #condition  = conditions[2] #idea generation
        pic_path = os.path.join(subpath_s, condition)
        if not os.path.exists(pic_path):
            os.makedirs(pic_path)
        for unit_no in range(64):
            htmlname = 'unit'+str(unit_no)+'.html'
            figname = 'unit'+str(unit_no)+'.jpg'
            visualize(hidden, data_set.train_data,unit_no, pic_path, htmlname,figname)
